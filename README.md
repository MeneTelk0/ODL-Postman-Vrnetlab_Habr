 ...
 ---
 Играем в SDN или как собрать виртуальную лабораторию при помощи OpenDaylight, Postman и Vrnetlab
 ---
...
````
Это внутренние пространсва имен XML (XML namespace) для ODL в соответствии с которыми он создает node.
Соотвественно, имя роутера - это node-id, адрес роутера - host и тд. Самая интересная строчка - последняя. 
Schema-cache-directory создает директорию, в которую выкачиваются все файлы YANG Schema подключенного роутера, если изначально она пустая. В противном случае, для вновь подключенного роутера будут использованы YANG Schema находящиеся в данной директории.
